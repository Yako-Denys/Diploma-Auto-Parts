...
export function FavoriteButton({ product, className = "" }: FavoriteButtonProps) {
  ...
  return (
    <Button variant="secondary" size="icon" onClick={() => mutate()} disabled={isPending} className={className}>
      {isExists ? <AiFillHeart color="#F43F5E" className="size-5" /> : <AiOutlineHeart className="size-5" />}
    </Button>
  )
}
