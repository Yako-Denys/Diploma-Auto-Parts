...
export class StoreService {
  ...
  async getById(storeId: string, userId: string) {
    const store = await this.prisma.store.findUnique({ where: { id: storeId, userId } })
    if (!store) throw new NotFoundException("Магазин не знайдений або ви не є його власником")
    return store
  }
  ...
  async update(storeId: string, userId: string, dto: UpdateStoreDto) {
    ...
    return this.prisma.store.update({ where: { id: storeId }, data: { ...dto, userId } })
  }
